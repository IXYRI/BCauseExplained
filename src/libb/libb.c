...
#ifndef B_FN
  /*
   * This macro allows each B std function to receive a pre- or postfix
   * to avoid issues with common names.
   *
   * e.g.: #define B_FN(name) __b##name <-- gives each B function the prefix "__b".
   * Default keeps the historical names; the old names are the point of the exhibit.
   */
...
static inline SYSCALL_TYPE __syscall3(SYSCALL_TYPE n, SYSCALL_TYPE a1, SYSCALL_TYPE a2, SYSCALL_TYPE a3) {
	/* This runtime only wraps the syscall arities it actually uses. */
	unsigned SYSCALL_TYPE ret;
...
/* Tiny arity dispatcher for syscall(...). */
...
static long strlen(char const *string) {
	long i = 0;
	while (string [i++]);
	return i - 1;
}

static void __assert_fail(char const *condition, char const *file, int const line, char const *func) {
...
/* B strings are word values holding byte addresses. */
B_TYPE      B_FN(_char)(B_TYPE string, B_TYPE i) { return (( char * ) string) [i]; }

/* The path name represented by the string becomes the current directory.
   A negative number returned indicates an error. */
B_TYPE      B_FN(chdir)(B_TYPE string) { return ( B_TYPE ) syscall(SYS_chdir, string); }

/* The file specified by the string has its mode changed to the mode argument.
   A negative number returned indicates an error. */
B_TYPE      B_FN(chmod)(B_TYPE string, B_TYPE mode) { return ( B_TYPE ) syscall(SYS_chmod, string, mode); }

/* The file specified by the string has its owner changed to the owner argument.
   A negative number returned indicates an error. */
B_TYPE      B_FN(chown)(B_TYPE string, B_TYPE mode) { return ( B_TYPE ) syscall(SYS_chown, string, mode); }

/* The open file specified by the file argument is closed.
   A negative number returned indicates an error */
B_TYPE      B_FN(close)(B_TYPE file) { return ( B_TYPE ) syscall(SYS_close, file); }
...
/* The file specified by the string is either truncated or
   ...
   indicates an error.  */
B_TYPE      B_FN(creat)(B_TYPE string, B_TYPE mode) { return ( B_TYPE ) syscall(SYS_creat, string, mode); }
...
static char month_strs [12][3] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

void        B_FN(ctime)(B_TYPE time_vec, B_TYPE date) {
	short  hour, minute, second;
	long   a, b, c, d, month, day;
	/* The B interface passes both vectors as word-valued addresses. */
	B_TYPE time      = *( B_TYPE * ) time_vec;
	char  *date_vec  = ( void * ) date;

	/* Break seconds since epoch into day time first. */
	second           = time % 60;
	time            /= 60;
	minute           = time % 60;
	time            /= 60;
	...
	/* Integer civil-calendar conversion. Compact, old-school, and not self-explanatory. */
	...
	/* Write exactly the documented "Mmm dd hh:mm:ss" byte layout. */
...
void B_FN(execl)(B_TYPE string, ...) {
	...
	/* Linux wants an envp vector; historical B interface does not pass one. */
	B_TYPE        envp;
	int           i = 0;
	...
	/* Copy variadic B words until the terminating zero argument. */
...
void B_FN(execv)(B_TYPE string, B_TYPE argv, B_TYPE count) {
	/* execv receives a B vector and builds the null-terminated argv expected by execve. */
	B_TYPE envp = 0;
	B_TYPE args [count + 1];
	for (B_TYPE i = 0; i < count; i++) args [i] = (( B_TYPE * ) argv) [i];
	args [count] = 0;
	syscall(SYS_execve, string, args, &envp);
}

/* The current process is terminated. */
void   B_FN(exit)(void) { syscall(SYS_exit, 0); }
...
/* The current process splits into two. The child process is
   ...
   an error. */
B_TYPE B_FN(fork)(void) { return ( B_TYPE ) syscall(SYS_fork); }

/* The i-node of the open file designated by file is put in
   the 20-word vector status. A negative number returned
   indicates an error. */
B_TYPE B_FN(fstat)(B_TYPE file, B_TYPE status) { return ( B_TYPE ) syscall(SYS_fstat, file, status); }
...
B_TYPE B_FN(getchar)(void) {
	char c;
	if (syscall(SYS_read, stdin, &c, 1) != 1) return 0;
	return c;
}

/* The user-ID of the current process is returned. */
B_TYPE B_FN(getuid)(void) { return ( B_TYPE ) syscall(SYS_getuid); }
...
B_TYPE B_FN(gtty)(B_TYPE file, B_TYPE ttystat) {
	/* Preserved API surface: Unix had it, Linux does not. */
	( void ) file;
	( void ) ttystat;
	return -1;
}
...
/* Counterpart to char(): write one byte through a word-valued address. */
void   B_FN(lchar)(B_TYPE string, B_TYPE i, B_TYPE chr) { (( char * ) string) [i] = chr; }
...
B_TYPE B_FN(link)(B_TYPE string1, B_TYPE string2) { return ( B_TYPE ) syscall(SYS_link, string1, string2); }

/* The directory specified by the string is made to exist with
   the specified access mode. A negative number returned
   indicates an error. */
B_TYPE B_FN(mkdir)(B_TYPE string, B_TYPE mode) { return ( B_TYPE ) syscall(SYS_mkdir, string, mode); }

B_TYPE B_FN(open)(B_TYPE string, B_TYPE mode) { return ( B_TYPE ) syscall(SYS_open, string, mode); }
...
void   B_FN(printf)(B_TYPE fmt, ...) {
	...
	va_start(ap, fmt);
loop:
	/* Copy literal bytes until a conversion introducer or NUL terminator. */
	...
	case 'd' : /* decimal */
	case 'o' : /* octal */
		/* All variadic arguments are B words. */
		x = va_arg(ap, B_TYPE);
		if (x < 0) {
			x = -x;
			B_FN(putchar)('-');
	...
	case 'c' :
		/* %c prints a B word through putchar(), so multi-character constants work. */
		x = va_arg(ap, B_TYPE);
		B_FN(putchar)(x);
		goto loop;

	case 's' :
		/* %s walks byte-by-byte using the B `char` primitive. */
		x = va_arg(ap, B_TYPE);
		j = 0;
		while ((c = B_FN(_char)(x, j++)) != '\0') B_FN(putchar)(c);
		goto loop;
	case '%' : B_FN(putchar)('%'); goto loop;
	}
	/* Unknown conversions are printed literally, then rescanned from the next byte. */
	B_FN(putchar)(c);
	i--;
	goto loop;
...
void B_FN(printn)(B_TYPE n, B_TYPE b) {
	...
	/* Print high digits first by recursion; tiny and very much of its era. */
	if ((a = n / b)) B_FN(printn)(a, b);
	B_FN(putchar)(n % b + '0');
}
...
void B_FN(putchar)(B_TYPE chr) {
	...
	/* A B character value may contain several bytes packed into one word. */
	u.word       = chr;
	/* Trim trailing zero bytes so 'foo' writes three bytes, not a full word. */
	while (len > 1 && u.c [len - 1] == 0) len--;
...
B_TYPE B_FN(setuid)(B_TYPE id) { return ( B_TYPE ) syscall(SYS_setuid, id); }

/* The i-node of the file specified by the string is put in
   the 20-word vector status. A negative number returned
   indicates an error. */
B_TYPE B_FN(stat)(B_TYPE string, B_TYPE status) { return ( B_TYPE ) syscall(SYS_stat, string, status); }
...
void   B_FN(time)(B_TYPE timev) { *(( B_TYPE * ) timev) = syscall(SYS_time, 0); }

/* The link specified by the string is removed. A negative
   number returned indicates an error. */
B_TYPE B_FN(unlink)(B_TYPE string) { return ( B_TYPE ) syscall(SYS_unlink, string); }
